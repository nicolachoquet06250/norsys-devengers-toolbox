...
func (f File) Exists() (exists bool, err error) {
	f.Path = strings.ReplaceAll(f.Path, environement.Slash()+environement.Slash(), environement.Slash())
...
func (f File) Create(content string, recursive bool) error {
	...
	var splitPath = strings.Split(f.Path, environement.Slash())
...
func (f File) Remove() error {
	return os.Remove(f.Path + environement.Slash())
}
